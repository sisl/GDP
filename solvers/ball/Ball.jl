...
function simulate(p::BallParams)
    ...
    @setObjective(ball, Min, cg*sum{X[i],i=1:T-1} + sum{p.p[q]*p.ca*sum{W[q,i],q=1:Q},i=1:T-1})
    ...
    for q = 1:Q
        constraint3[q] = @addConstraint(ball, W[q,0] == 0)
    end 
    ...
    constraint5[1] = @addConstraint(ball, X[0] == 0)
    ...
    return GDPAction(int16(delayed[2:end-1])')::GDPAction # 1 empty interval
